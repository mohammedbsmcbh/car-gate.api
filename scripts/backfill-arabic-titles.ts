...
// Arabic translations for car brands (foreign brands stay as-is)
const ARAB_BODY_TYPES: Record<string, string> = {
...
function buildArabicDescription(listing: any): string {
  if (listing.type === 'PLATE') {
...
async function main() {
  ...
  for (const listing of listings) {
    ...
    updated++;
    if (updated % 10 === 0) process.stdout.write(`\r  Updated ${updated}/${listings.length}...`);
  }
