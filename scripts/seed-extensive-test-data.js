...
async function createListing(user, titleSuffix, type = client_1.ListingType.CAR, agencyId, showroomId) {
    ...
    let specificData = {};
    if (type === client_1.ListingType.CAR) {
        specificData = {
            make: brand,
            model: model,
    ...
    if (type === client_1.ListingType.BIKE) {
    ...
    else if (type === client_1.ListingType.PLATE) {
    ...
    await prisma.listing.create({
        data: {
            ...
            media: {
                create: [
                    {
                        url: type === client_1.ListingType.PLATE
...
async function main() {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        const user = await prisma.user.upsert({
            where: { email },
            create: {
                email, password: hashedPassword, name: `Agency Manager ${i}`, role: client_1.UserRole.AGENCY,
        ...
        if (user.agency) {
            await createListing(user, `Owner Post`, client_1.ListingType.CAR, user.agency.id);
            for (let j = 1; j <= 5; j++) {
                ...
                await createListing(user, `Employee ${j} Post`, client_1.ListingType.CAR, user.agency.id);
            }
        }
    }
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        const user = await prisma.user.upsert({
            where: { email },
            create: {
                email, password: hashedPassword, name: `Showroom Boss ${i}`, role: client_1.UserRole.SHOWROOM,
        ...
        if (user.showroom) {
            await createListing(user, `Owner Post`, client_1.ListingType.CAR, undefined, user.showroom.id);
            for (let j = 1; j <= 5; j++) {
                ...
                await createListing(user, `Employee ${j} Post`, client_1.ListingType.CAR, undefined, user.showroom.id);
            }
        }
    }
    console.log('\n👔 Processing Traders...');
    for (let i = 1; i <= 5; i++) {
        ...
        const user = await prisma.user.upsert({
            where: { email },
            create: {
                email, password: hashedPassword, name: `Trader ${i}`, role: client_1.UserRole.TRADER,
                isApproved: true, isActive: true, phone: `3320000${i}`
            },
            update: {}
        });
        await createListing(user, `Trader Stock`, client_1.ListingType.CAR);
    }
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        const user = await prisma.user.upsert({
            where: { email },
            create: {
                email, password: hashedPassword, name: `Individual ${i}`, role: client_1.UserRole.INDIVIDUAL,
                isApproved: true, isActive: true, phone: `3340000${i}`
            },
            update: {}
        });
        await createListing(user, `Personal Car`, client_1.ListingType.CAR);
    }
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        const user = await prisma.user.upsert({
            where: { email },
            create: {
                email, password: hashedPassword, name: `Clearing Agent ${i}`, role: client_1.UserRole.TRADER,
        ...
        await createListing(user, `Clearance Service Car`, client_1.ListingType.CAR);
    }
    console.log('\n🏍️  Adding Bikes & Plates...');
    const randomUser = await prisma.user.findFirst({ where: { role: client_1.UserRole.INDIVIDUAL } });
    if (randomUser) {
        for (let k = 0; k < 5; k++) {
            await createListing(randomUser, `Bike Listing ${k + 1}`, client_1.ListingType.BIKE);
            await createListing(randomUser, `Special Plate ${k + 1}`, client_1.ListingType.PLATE);
        }
    }
...
//# sourceMappingURL=seed-extensive-test-data.js.map