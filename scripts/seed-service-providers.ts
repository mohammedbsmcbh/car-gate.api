import { PrismaClient, ApprovalStatus, BookingStatus, ServiceRequestStatus } from '@prisma/client';
...
// ─── Sample customers & bookings data ─────────────────────────────────────────
...
const SAMPLE_BOOKINGS = [
  { customerName: 'Ahmed Al Khalid', customerPhone: '+973 33111222', date: '2026-03-05', time: '10:00 AM', status: BookingStatus.PENDING },
  { customerName: 'Sara Al Mansoori', customerPhone: '+973 36222333', date: '2026-03-08', time: '02:30 PM', status: BookingStatus.CONFIRMED, notes: 'Please check the AC as well' },
  { customerName: 'Mohammed Hassan', customerPhone: '+973 38333444', date: '2026-02-20', time: '11:00 AM', status: BookingStatus.COMPLETED },
  { customerName: 'Fatima Al Zaabi', customerPhone: '+973 33445566', date: '2026-03-10', status: BookingStatus.PENDING },
];

const SAMPLE_REQUESTS = [
  { customerName: 'Ahmed Al Khalid', customerPhone: '+973 33111222', requestType: 'new_car_clearance', status: ServiceRequestStatus.PENDING, notes: '2024 Toyota Land Cruiser from Japan' },
  { customerName: 'Sara Al Mansoori', customerPhone: '+973 36222333', requestType: 'ownership_transfer', status: ServiceRequestStatus.IN_PROGRESS, notes: 'Transfer from father to son' },
  { customerName: 'Khalid Al Dosari', customerPhone: '+973 37556677', requestType: 'plate_registration', status: ServiceRequestStatus.COMPLETED },
  { customerName: 'Noura Al Sayed', customerPhone: '+973 39887766', requestType: 'used_car_clearance', status: ServiceRequestStatus.PENDING, notes: 'BMW 5 Series 2022 from USA' },
];
...
async function main() {
  ...
  // Create a dummy customer user ID for bookings
  ...
  for (let i = 0; i < INSPECTION_CENTERS.length; i++) {
    ...
    for (const svc of c.services) {
      ...
      if (!existing) {
        ...
        // Add a sample booking on the first inspection center's first service
  ...
  for (let i = 0; i < POLISHING_CENTERS.length; i++) {
    ...
    for (const svc of c.services) {
      ...
      if (!existing) {
        ...
        // Add a sample booking on the first polishing center's first two services
  ...
  for (let i = 0; i < CUSTOMS_CLEARERS.length; i++) {
    ...
    // Add sample service requests to first customs clearer
    if (i === 0) {
